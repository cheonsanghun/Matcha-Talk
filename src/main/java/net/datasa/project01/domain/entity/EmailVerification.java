 ...
 *  - user_pid: FK → users.user_pid (사용자 테이블 참조)
...
@Table(
        ...
        indexes = {
                @Index(name = "idx_ev_user_purpose", columnList = "user_pid, purpose, expires_at") // 복합 인덱스 생성
        },
        uniqueConstraints = {
...
public class EmailVerification {
    ...
    /** FK: 대상 사용자 */
    @ManyToOne(fetch = FetchType.LAZY, optional = false) // User 엔티티와 다대일 관계, 지연 로딩
    @JoinColumn(
            name = "user_pid", // 외래키 컬럼명
            foreignKey = @ForeignKey(name = "fk_ev_user"), // 외래키 제약조건 이름
            nullable = false // null 불가
    )
    private User user; // 인증 대상 사용자 정보
